  ...
  const [loading, setLoading] = useState(false);
   ...
   const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(t('campaigns.createdSuccessfully'));
      onSave();
    } catch (error: any) {
      console.error(error);
      alert(t('campaigns.creationError') + ": " + (error.message || error));
    } finally {
      ...
      <div className="bg-white border-b border-slate-200 px-6 pt-8 pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold text-slate-900">{t('campaigns.create')}</h1>
          </div>
          <LanguageSelector />
        </div>
      </div>
