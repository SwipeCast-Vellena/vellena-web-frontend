      ...
      try {
        ...
        if (data.success) {
          ...
          if (!selectedChat && recentChats.length > 0) {
            ...
            navigate(`/model/chat/${recentChats[0].chatId}`, { replace: true });
          }
        }
      } catch (err) {
            ...
            >
              {chat.avatarUrl ? (
                ...
                <div
                  ...
                  style={{ backgroundColor: getAvatarColor(chat.agencyName) }}
                >
                  {chat.agencyName?.charAt(0).toUpperCase() || "?"}
                </div>
              )}
              <div className="flex-1">
                <p className="font-medium text-slate-900">{chat.title}</p>
                <p className="text-l text-slate-400">{chat.agencyName}</p>
